...
def run_calculator(income, expenses, loan, rate, months):
    ...
    return dict(
        ...
        status=s, conclusion=c, message_hi=mh, message_en=me
    )
...
def user_register():
    ...
    try:
        db.execute("""
            INSERT INTO users
              (session_id,name,age,mobile,vocation,vocation_custom,city,state,ip_address,user_agent)
            VALUES (?,?,?,?,?,?,?,?,?,?)
        ...
        db.execute("INSERT INTO app_events (user_id,event,meta) VALUES (?,?,?)",
                   (uid,"register",json.dumps({"vocation":d.get("vocation")})))
        db.commit()
...
def user_calculate():
    ...
    db = get_db(); uid = d.get("user_id"); voc = d.get("vocation","unknown")
    try:
        db.execute("""
        ...
        db.execute("INSERT INTO app_events (user_id,event,meta) VALUES (?,?,?)",
                   (uid,"assessment",json.dumps({
                       ...
                       "loan_source":source,"lti":r["lti_ratio"]})))
